...
;20170521  T.A.         ; orca_idle, orca_capturemode_sequence before orcaobs in Normalobs & PolObs, I didn't understand reason
;
;
;========================headder==============================;
;function 	timeinit
	...
	1:begin		;ORCA-Flasch4.0
		...
		endwhile
		  ;print,get_systime(ctime=enctime)   ;~26sec
			...
			;print,get_systime(ctime=enctime)   ;0sec
                        orca_idle                                   ; 20170521 T.A.
                        orca_capturemode_sequence,nimg=wp.nimg      ; 20170521 T.A.
			imgs=OrcaObs(nimg=wp.nimg,/grabimg_wait)
			;print,get_systime(ctime=enctime)   ;~23sec
			p.date_obs2=get_systime(ctime=enctime)
...
;20161111  t.a.         ;add index 'AZ' and 'IMGROT'
;20170522  t.a.         ;change orca4 part
;--------------------------------------------------------------
COMMON bridge,bridge
...
case wp.camera of
	...
	1:begin		;ORCA-Flasch4.0
		;p=orcainit() 					;comment out 20160709 T.A.
		;=> 2017.05.22 T.A.
		; I don't understand orca_capturemode_sequence before orca_capturemode_snap
                orca_idle 
                orca_capturemode_sequence,nimg=wp.nimg &wait,0.1
		orca_idle    ;capture stop
		orca_capturemode_snap,nimg=nimg &wait,0.1

		; We need to setting trigger after orca_capturemode_snap
		p=OrcaSetParam(expo=wp.expo*1e-6,bin=wp.binx)
		OrcaOutTriggerExposure
		orca_settrigmode,'Start',polarity='Negative'
		date_obs=get_systime(ctime=stctime)
		print,'trigger waiting, ',date_obs & wait,0.1
