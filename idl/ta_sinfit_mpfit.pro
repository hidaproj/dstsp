...
;
; PURPOSE:
;   Fitting by sinusoidal curve with Levenberg-Marquardt least-squares minimization (MINPACK-1)
;	
;	DAT = AV + AMP * sin(K * XX + PH)
;
; CATEGORY:
; CALLING SEQUENCE:
;   yfit = ta_sinfit_mpfit(xx,dat,av=av,amp=amp,k=k,ph=ph)
; INPUTS:
;   XX   = a vector of independent variables
;   DAT  = a vector of dependent variables, the same length as XX
; OUTPUTS:
;   YFIT = varue of function at each XX
;
...
;   TPRINT  = print information of fitting
; HISTORY
;   2014.7.17 T.A.
;   2014.9.1  T.A.   add keyword 'MAXITER' and 'TPRINT'
;   2014.11.21 T.A.  modify initial estimate for ph  
;   2016.08.24 T.A.  estimates
;-
...
;	If no frequency range is specified, go from f_min = 1/T
;	to a mock Nyquist frequency gotten from the mean spacing of
;	data points, rather than the spacing of a fixed grid as usual
...
;
;	be verbose if need be
;
if keyword_set(noverbose) eq 0 then begin 
  print,'Now computing a power spectrum of ', fix(n_points), ' points'
  print,'   f_min = ', freqrange(0), ' Hz'
  print,'   f_max = ', freqrange(1), ' Hz'
  print,'   df    = ', deltaf, ' Hz' 
  print,'The effective Nyquist frequency is ',fnyquist,' Hz'
  print,'Mean and sigma of time series, ', mseries, rtvar 
endif
;
;	compute power spectrum
;
for j = 1, n_points-2 do begin
  i = j-1
  freq(i) = freqrange(0) + j * deltaf
  sinfunct = sin(2*!pi * freq(i) * secs)
  cosfunct = cos(2*!pi * freq(i) * secs)
  sinparams = poly_fit(sinfunct,tseries,1,sinfit,sin_sigma)
  cosparams = poly_fit(cosfunct,tseries,1,cosfit,cos_sigma)
  power(i) = (sinparams(1)^2 + cosparams(1)^2)/2/deltaf
endfor
if keyword_set(noverbose) eq 0 then begin 
  print, 'Parseval factor is ', total(power) * deltaf/rtvar^2
endif
;stop
end 
;=============================================================================
...
noverbose=1
if keyword_set(tprint) then noverbose=0
...
p0=[av,a,k,ph]
if keyword_set(estimates) then p0=estimates
yfit0=fun(xx,p0)
